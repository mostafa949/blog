...
@section('content')
    ...
    @foreach($posts as $post)
        <div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200">
            <div>
                <img src="{{ postStorageFilePath($post->image_path) }}" width="500" alt="">
            </div>
            <div>
               ...
               By <span class="font-bold italic text-gray-800">{{$post->admin->name}}</span>, Created on {{date('jS M Y', strtotime($post->updated_at))}}<br>
                ...
                <a
                    href="{{ route('blog.posts.show', ['post' => $post->slug]) }}"
                ...
                @auth('admin')
                    <span class="float-right">
                        <a
                            href="{{ route('blog.posts.edit', ['post' => $post->slug]) }}"
                            class="text-gray-700 italic hover:text-gray-900 pb-1 border-b-2">Edit</a>
                    </span>

                    <span class="float-right">
                        <form
                            action="{{ route('blog.posts.destroy', ['post' => $post->slug]) }}"
                            method="post">
                            @csrf
                            @method('delete')
